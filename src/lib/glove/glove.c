...
void *glove_thread(void *vid) {
    ...
    for (a = 0; a < lines_per_thread[id]; a++) {
        ...
        if (isnan(diff) || isnan(fdiff) || isinf(diff) || isinf(fdiff)) {
            fprintf(logfile,"Caught NaN in diff for kdiff for thread. Skipping update");
            continue;
        }
...
int save_params(int nb_iter) {
    ...
    if (use_binary > 0) { // Save parameters in binary file
        ...
        if (fout == NULL) {fprintf(logfile, "Unable to open file %s.\n",save_W_file); return 1;}
        ...
        if (save_gradsq > 0) {
            ...
            if (fgs == NULL) {fprintf(logfile, "Unable to open file %s.\n",save_gradsq_file); return 1;}
    ...
    if (use_binary != 1) { // Save parameters in text file
        ...
        if (fid == NULL) {fprintf(logfile, "Unable to open file %s.\n",vocab_file); return 1;}
...
int train_glove() {
    ...
    fin = fopen(input_file, "rb");
    if (fin == NULL) {fprintf(logfile,"Unable to open cooccurrence file %s.\n",input_file); return 1;}
    ...
    for (b = 0; b < num_iter; b++) {
        ...
        fprintf(logfile, "%s, iter: %03d, cost: %lf\n", time_buffer,  b+1, total_cost/num_lines);

        if (checkpoint_every > 0 && (b + 1) % checkpoint_every == 0) {
            fprintf(logfile,"    saving itermediate parameters for iter %03d...", b+1);
...
int train(char* input_file_, char* vocab_file_, char* output_vector_files, int do_save_gradsq_files, char* opt_output_gradsq_files, int verbosity, char *log_file) {
    ...
    fid = fopen(vocab_file, "r");
    if(fid == NULL) {
        fprintf(logfile, "Unable to open vocab file %s.\n",vocab_file); 
        return 1;
    }
